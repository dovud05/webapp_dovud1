...
DB_HOST = "db-dovud3t.c32geugqgvkj.ap-southeast-1.rds.amazonaws.com"
DB_NAME = "db_dovud"
DB_USER = "postgres"
...
def get_transactions():
    try:
        ...
        base_query = 'FROM tbl_dovud_supermarket_sales WHERE 1=1'
        params = []
...
def add_inventory():
    try:
        ...
        cursor.execute('SELECT 1 FROM tbl_dovud_supermarket_sales WHERE "Invoice ID" = %s',
                     (data["Invoice ID"],))
        if cursor.fetchone():
            cursor.close()
            conn.close()
        ...
        # Insert new record
        cursor.execute('''
            INSERT INTO tbl_dovud_supermarket_sales
...
def remove_inventory(invoice_id):
    try:
        ...
        cursor.execute('SELECT * FROM tbl_dovud_supermarket_sales WHERE "Invoice ID" = %s', (invoice_id,))
        ...
        # Delete the record
        cursor.execute('DELETE FROM tbl_dovud_supermarket_sales WHERE "Invoice ID" = %s', (invoice_id,))
...
def get_stats():
    try:
        ...
        cursor.execute('SELECT SUM("Total Sales") FROM tbl_dovud_supermarket_sales')
        total_sales_amount = cursor.fetchone()[0]

        # Get total number of transactions
        cursor.execute('SELECT COUNT(*) FROM tbl_dovud_supermarket_sales')
        total_transactions = cursor.fetchone()[0]

        # Get sales by category
        cursor.execute('SELECT "Product Category", SUM("Total Sales") as total FROM tbl_dovud_supermarket_sales GROUP BY "Product Category" ORDER BY total DESC')
        sales_by_category = [dict(row) for row in cursor.fetchall()]

        # Get sales by payment method
        cursor.execute('SELECT "Payment Method", COUNT(*) as count FROM tbl_dovud_supermarket_sales GROUP BY "Payment Method" ORDER BY count DESC')
